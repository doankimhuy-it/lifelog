...
const TagListControlButton: React.FC<ControlButton> = ({
  ...
  if (is_left) {
    return (
      <div
        ...
        className={cn(
          'absolute items-center justify-center left-0 bg-grayish-50 rounded-full flex w-6 h-6 cursor-pointer',
          className,
        )}
      >
        <GoLeft></GoLeft>
  ...
  return (
    <div
      ...
      onClick={handleScroll}
      className={cn(
        'absolute right-0 items-center justify-center bg-grayish-50 rounded-full flex w-6 h-6 cursor-pointer',
        className,
      )}
    >
      <GoRight></GoRight>
