...
const CircleButton = ({
  ...
  return (
    <button
      ...
      className={cn(
        className,
        `relative p-[1.5px] bg-grayish-70 select-none cursor-pointer ${disabled ? 'pointer-events-none disabled ' : ' active:bg-grayish-90 hover:bg-linear-hover '} rounded-[32px] before:content-[''] before:absolute before:inset-0 before:opacity-25 before:bg-linear-card before:rounded-[32px] after:content-[''] after:absolute after:inset-[1.5px] after:bg-[#313131] after:rounded-[30.5px]`,
      )}
    >
      <span
        className={cn(
