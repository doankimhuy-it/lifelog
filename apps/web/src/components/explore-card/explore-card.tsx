...
export default function ExploreCard({ explore }: ExploreCardProps) {
  return (
    <Link
      to={`/explore/${explore.id}`}
      className="group w-full min-w-[17.5rem] flex flex-col rounded-[20px] p-3 bg-reddish-2 gap-3 md:items-start md:justify-center md:hover:bg-[#f8f8f81a]"
    >
      <img
      ...
      <div className="trending-content grow flex flex-col gap-2 mx-2 md:justify-start md:items-start opacity-80">
        <Typography level="base-m" className="text-primary line-clamp-1">
          {explore.title}
        </Typography>
        <Typography level="caption" className="text-secondary line-clamp-2">
          {explore.description}
        </Typography>
      </div>
      ...
      <div className="flex w-full justify-start items-center gap-3 px-3">
        <AvatarGroup
          ...
          className="size-[1.25rem] min-w-[1.25rem] border-reddish-2"
        />
