  ...
  return (
    <NavLink
      ...
      className={({ isActive }: { isActive: any; isPending: any }) =>
        ` relative z-0  flex items-center justify-center rounded-xl ${isActive ? `active p-[1px] bg-reddish-10 rounded-xl before:absolute before:inset-0 before:opacity-15 before:bg-linear-card before:rounded-xl after:content-[''] after:absolute after:inset-[1px] after:bg-[#313131] after:rounded-[11px] ` : ` `}`
      }
    >
      <div
        className={`group-[.active]:bg-reddish-10 group-[.active]:p-[5px] flex items-center  p-1.5 rounded-[11px] relative z-1 backdrop-blur-16`}
      >
